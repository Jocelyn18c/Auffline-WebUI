...
inline void handleSettingsNav(NavEvent e) {
  if (e == NAV_UP || e == NAV_LEFT) {
    if (settingsSelectedIndex <= 0) return;
    settingsSelectedIndex--;
    scrollToSettingsSelection();
    updateSettingsMenuSelection();
  }
  else if (e == NAV_DOWN || e == NAV_RIGHT) {
    if (settingsSelectedIndex >= SETTINGS_COUNT - 1) return;
    settingsSelectedIndex++;
    scrollToSettingsSelection();
    updateSettingsMenuSelection();
  }
    ...
    switch (settingsSelectedIndex) {
      case SETTINGS_BLUETOOTH:
        ...
        navigateTo(SCREEN_BLUETOOTH);
        break;
      case SETTINGS_WIFI:      navigateTo(SCREEN_WIFI);     break;
      case SETTINGS_DISPLAY:   navigateTo(SCREEN_DISPLAY);  break;
      case SETTINGS_AUDIO:     navigateTo(SCREEN_AUDIO);    break;
      case SETTINGS_ABOUT:     navigateTo(SCREEN_ABOUT);    break;
    }
  }
  else if (e == NAV_BACK) {
    navigateTo(SCREEN_MENU);
